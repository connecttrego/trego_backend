...
public interface OrderItemRepository extends JpaRepository<OrderItem, Long> {
    @Query("SELECT oi.vendorMedicineId, COUNT(oi) as salesCount " +
            "FROM OrderItem oi " +
            "GROUP BY oi.vendorMedicineId " +
...
//    List<Object[]> findTopSellingMedicineIdsByVendor(@Param("vendorId") Long vendorId);
    ...
    @Query("""
                SELECT oi.order.vendor.id as vendorId, oi.vendorMedicineId as medicineId,
                       COUNT(oi.id) as salesCount, MAX(oi.mrp) as mrp, MAX(oi.qty) as qty
                FROM OrderItem oi
                GROUP BY oi.order.vendor.id, oi.vendorMedicineId
